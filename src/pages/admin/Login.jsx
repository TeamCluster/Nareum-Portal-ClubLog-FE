 ...
 *
 * 흐름:
 *   - 마운트 시 GET /api/admin/session 으로 로그인 상태 확인 ->
 *     이미 로그인됐으면 /setting/main 으로 자동 이동
 *   - 비밀번호 제출 -> POST /api/admin/login
 *   - 성공: /setting/main 으로 이동, 실패: 에러 메시지
 */
export default function Login() {
  ...
  useEffect(() => {
    apiGet('/admin/session')
      .then((data) => {
        if (data.logged_in) navigate('/setting/main', { replace: true })
      })
      .catch(() => {
        // 세션 확인 실패해도 로그인 폼은 그대로 노출
      })
  }, [navigate])
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      navigate('/setting/main', { replace: true })
    } catch (err) {
